...
- **Apply for Jobs**:
  ...
  - If a job has **Screening Questions** (defined by the employer), you'll answer them (text, yes/no supported initially) during the application process via a modal. Answers are final once submitted.
  - Once an application is submitted (even if later withdrawn or rejected), you cannot re-apply for the same job.
  - _Note_: Applying is disabled if your account is 'suspended'.
- **Withdraw Application**:
  - From the "My Jobs" page or the job detail page (if applied), you can withdraw an active application (e.g., one with 'Applied' status). This action requires confirmation.
  - Withdrawn applications cannot be re-submitted.
...
- **My Jobs Page (`/my-jobs`)**:
  - A centralized dashboard to view and manage:
    ...
    - **Applied Jobs**: Jobs you've submitted applications for, showing their current status (e.g., Applied, Reviewed, Withdrawn by Applicant, Rejected By Company).
...
- _Note_: Disabled if your account is 'suspended'. Jobs for which you have an active, withdrawn, or rejected application might still appear here, but the "Apply" button on the job detail page will be disabled for them.
...
graph TD
    ...
    CheckExistingApp -- No --> N_Pre[Initiate Apply - Disabled if Suspended]
    ...
    CheckExistingApp -- Yes --> N_Disabled[Show "Cannot Re-apply" / "Already Applied"]
    N_Disabled --> L
    N --> P[View in My Jobs Applied: /my-jobs]
    P --> Q[Await Employer Response or Withdraw]
    Q -- Withdraw? --> Withdraw_Confirm[Confirm Withdraw]
...
| `/jobs/[jobId]`          | View job details. Apply/Save actions disabled if account 'suspended'. Re-application prevented.           | Public       |
...
- **Profile & Application Data (Firebase Firestore):**
  ...
  - **Job Application**: An `application` document is created in the `applications` collection, now potentially including `answers` to screening questions. Disabled if account 'suspended'. Re-application to the same job by the same user is prevented.
  - **Withdraw Application**: Updates the `status` of an existing `application` document to `'Withdrawn by Applicant'`.

 ...
 *
 * It takes a job description and a list of candidate profiles (including detailed work experience, education, skills, preferences like salary and location),
...
const AIPoweredCandidateMatchingOutputSchema = z.object({
  ...
  reasoning: z
    .string()
    .describe(
      'A detailed explanation of why these specific candidates were selected. For each candidate, highlight how their skills, experience, education, salary expectations, and preferences align with the job description. Mention any potential misalignments if significant but outweighed by other factors.'
    ),
});
...
1.  Thoroughly analyze the job description, noting key responsibilities, required and preferred skills, experience level, location, job type, and salary range (if provided).
2.  Carefully review each candidate's profile. Pay close attention to their skills, detailed work experience (roles, responsibilities, duration), education, stated preferences (preferred locations, desired salary in INR, job search status, availability), and any resume summary.
3.  Identify the candidate UIDs that are the MOST relevant matches for the job description.
4.  Provide a detailed reasoning for your selections. For each recommended candidate, explain how their profile aligns with the job's requirements. Highlight specific matches in skills, experience, education, salary expectations (if their desired salary is compatible with the job's range or market rates), and location preferences. Also, note any potential minor misalignments if the overall match is strong.
...
Consider factors like years of experience, specific technical skills, cultural fit (if discernible), and alignment of preferences (salary, location).

 ...
 *
 * It takes a job seeker's detailed profile (including skills, experience, education, preferences like salary and location)
...
const AIPoweredJobMatchingOutputSchema = z.object({
  ...
  reasoning: z
    .string()
    .describe(
      "A detailed explanation of why these specific jobs were selected for the seeker, highlighting key matches between the seeker's profile (skills, experience, salary expectations, location preferences) and the job requirements."
    ),
});
...
Your task is to:
1.  Thoroughly analyze the job seeker's profile, paying close attention to their skills, work experience (including roles, responsibilities, and duration), education, stated preferences (such as preferred locations, desired salary in INR, and job search status), and any summary from their resume.
...
4.  Provide a detailed reasoning for your selections. Explain for each recommended job (or generally for the set of recommendations) how it aligns with the seeker's profile. Highlight specific connections, e.g., "The seeker's experience in 'Project Management' and skill 'Agile' directly match Job ID XYZ's requirements." or "Job ID ABC aligns with the seeker's desired salary range and preferred remote work option."
